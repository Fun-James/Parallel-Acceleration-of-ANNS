...
/* ---------------- SIMD 版 L2 距离 ---------------- */
static inline float l2_neon(const float* a, const float* b, int dim)
{
    ...
    for (; i + 15 < dim; i += 16) {
        ...
        vacc = vmlaq_f32(vacc,d0,d0);
        vacc = vmlaq_f32(vacc,d1,d1);
        vacc = vmlaq_f32(vacc,d2,d2);
        vacc = vmlaq_f32(vacc,d3,d3);
    }
    for (; i + 3 < dim; i += 4) {
        float32x4_t da = vsubq_f32(vld1q_f32(a+i), vld1q_f32(b+i));
        vacc = vmlaq_f32(vacc, da, da);
    }
    float32x2_t low = vadd_f32(vget_low_f32(vacc), vget_high_f32(vacc));
    float sum = vget_lane_f32(low,0) + vget_lane_f32(low,1);
             ...
             int          rerank_k = 0)
{
        ...
        for (int pi = 0; pi < real_probe; ++pi) {
            ...
            for (int m=0; m<M; ++m) {
                ...
                for (int k2=0; k2<K_pq; ++k2) { // Use K_pq
                    ...
                    // 使用 l2_neon 优化子距离计算
                    lut[m*K_pq + k2] = l2_neon(qs, c, sub_dim); // Use K_pq
                }
            }
    ...
    if (rerank_k > 0 && k > 0) { // Ensure rerank_k and k are positive for meaningful reranking
        ...
        for (int i=0;i<R;++i){
            ...
            float dist = l2_neon(query, vec, static_cast<int>(dim));
