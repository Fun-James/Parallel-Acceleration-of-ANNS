...
struct PQIndex {
    ...
    alignas(ALIGNMENT) uint8_t LUT_q8[PQ_M * PQ_K]{};
    ...
    void build_fastscan_layout() {
        ...
        size_t alloc_size = (total_size + ALIGNMENT - 1) / ALIGNMENT * ALIGNMENT;

      
        codes_fs = (uint8_t*)aligned_alloc(ALIGNMENT, alloc_size);
...
std::priority_queue<
    ...
    ~PQIndex(){
        if (codes_fs) {
            // --- 修改内存释放 ---
            free(codes_fs); // 使用 free 释放 aligned_alloc 分配的内存
            // --- 结束修改 ---
        }
    }
};
