...
// 优化的欧氏距离计算函数 - 一次处理8个浮点数
inline float neon_l2sqr(const float* x, const float* y, size_t d) {
    ...
    for (; i + 7 < d; i += 8) {
        // 第一个寄存器处理4个元素
        float32x4_t x_vec1 = vld1q_f32(x + i);
        float32x4_t y_vec1 = vld1q_f32(y + i);
        float32x4_t diff1 = vsubq_f32(x_vec1, y_vec1);
        
        // 第二个寄存器处理另外4个元素
        float32x4_t x_vec2 = vld1q_f32(x + i + 4);
        float32x4_t y_vec2 = vld1q_f32(y + i + 4);
        float32x4_t diff2 = vsubq_f32(x_vec2, y_vec2);
    ...
    for (; i + 3 < d; i += 4) {
        ...
        sum_vec1 = vmlaq_f32(sum_vec1, diff, diff);
    }
...
public:
    ...
    void build(float* data, size_t numPoints, size_t dim, int max_iterations = 200) {
        ...
        if (use_simd) {
            std::cout << "SIMD optimization (ARM Neon, 8-float processing) enabled" << std::endl;
        }
