...
#include <fstream>
#include <set>
#include <chrono> // 使用 chrono 替代 sys/time.h
#include <iomanip>
#include <sstream>
#include <omp.h>
#include <cmath> // <--- 添加 cmath 头文件以包含 INFINITY
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < test_number; ++i) {
        ...
        // 计算时间差，单位为微秒 (microseconds)
        int64_t diff = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count();
